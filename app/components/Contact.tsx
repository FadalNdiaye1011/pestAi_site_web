...
// Définir le type pour le statut de soumission
type SubmitStatus = {
  success: boolean;
  message: string;
} | null;
...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulation d'envoi (remplacez par votre propre logique si nécessaire)
    try {
      await new Promise(resolve => setTimeout(resolve, 1000));

      setSubmitStatus({
        success: true,
        message: 'Message envoyé avec succès! (Note: ceci est une démo, aucun message n\'a été réellement envoyé)'
      });
      setFormData({ name: '', email: '', message: '' });
    } catch (error) {
      setSubmitStatus({
        success: false,
        message: "Erreur lors de la simulation d'envoi"
      });
    } finally {
      setIsLoading(false);
    }
